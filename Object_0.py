...
def main():
    ...
    if choice == '1':
        ...
        model = CustomLongformer(config,adaptive_span_config)
    ...
    optimizer = AdamW([p for n, p in model.named_parameters() if 'alpha' not in n and 'adaptive_span' not in n],
                      lr=0.0001)
    attn_modules = [
        layer.attention.self
        for layer in model.longformer.encoder.layer
